                ...
                case Perk.EffectType.EntryPoint:
                    ...
                    switch (func)
                    {
                        ...
                        case APerkEntryPointEffect.FunctionType.AddLeveledList:
                            if (!payload.EPFT.HasValue) throw new MalformedDataException($"{nameof(PerkEntryPointAddLeveledItem)} did not have expected EPFT record");
                        ...
                        case APerkEntryPointEffect.FunctionType.AddActivateChoice:
                            ...
                            if (!payload.EPF3.HasValue) throw new MalformedDataException($"{nameof(PerkEntryPointAddActivateChoice)} did not have expected EPF3 record");
                        ...
                        case APerkEntryPointEffect.FunctionType.SelectSpell:
                            if (!payload.EPFT.HasValue) throw new MalformedDataException($"{nameof(PerkEntryPointSelectSpell)} did not have expected EPFT record");
